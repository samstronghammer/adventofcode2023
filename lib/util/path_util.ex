defmodule AdventOfCode.PathUtil do
  ...
  def dijkstra(start, is_goal, get_neighbors) do
    a_star(start, is_goal, fn _ -> 0 end, get_neighbors)
  end

  def a_star(start, is_goal, heuristic, get_neighbors) do
    frontier = Heap.new(fn {_v1, f1}, {_v2, f2} -> f1 < f2 end)
    frontier = Heap.push(frontier, {start, heuristic.(start)})
    came_from = Map.new()
    g_score = Map.new([{start, 0}])
    seen = MapSet.new()
    a_star_rec(is_goal, heuristic, get_neighbors, frontier, came_from, g_score, seen)
  end
